...
exports.getHospital = catchAsync(async (req, res, next) => {
    ...
    const hospital = await Hopital.findOne({_id:ObjectId(id)});
...
exports.modifyHospital = catchAsync(async (req, res, next) => {
    ...
    const id = req.params.id || null   
     ...
     await Hopital.findOneAndUpdate({_id:id},req.body);
    const hospital = await Hopital.findOne({_id:id});
...
exports.deleteHospital = catchAsync(async (req, res, next) => {
    ...
    const hospital = await Hopital.findOneAndDelete({_id:id},(err,ress)=>{
        if(err){
            return next(new AppError("n'a pas pu supprimer l'hôpital", err.status));
        }

        res.status(200).json({success:"hôpital supprimé avec succès"})
